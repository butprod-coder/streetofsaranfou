...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {JO_BRANCHES}=await import('/game/jo-talents.js');
    ...
    await Promise.all(["pallet","fire","weasel","fx"].map(name=>assets.load(`/assets/heroes/talents/jo/${name}.png`)));
    ...
    for(let branch=0;branch<3;branch++)for(const rank of [1,6]){
      ...
      applyProfile(p,{milestones:TRANSFORMATION_MILESTONES,talents:JO_BRANCHES[branch].nodes.slice(0,rank).map(n=>n.id)});
      ...
      const key=["joPallet","joFire","joWeasel"][branch];
    ...
    window.joQA={canvas,renderEvolution,profile:{milestones:TRANSFORMATION_MILESTONES,talents:[JO_BRANCHES[1].nodes[0].id]}};
    return stats;
  });
  assert.ok(result.every(s=>s.transparent>.2));
  await page.screenshot({path:'test-results/jo-transformations.png'});
  await page.evaluate(()=>{window.joQA.canvas.remove();document.querySelectorAll('.screen').forEach(e=>e.classList.remove('active'));document.querySelector('#evolution').classList.add('active');window.joQA.renderEvolution('jo',window.joQA.profile,()=>{});});
  ...
  await page.screenshot({path:'test-results/jo-talents-lock.png'});
