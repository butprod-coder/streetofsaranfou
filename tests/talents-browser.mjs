...
try {
  ...
  assert.equal(await page.locator('.talent-art svg').count(), 36);
  ...
  await screen(page, 'evolution'); await page.waitForFunction(() => document.activeElement?.dataset.talent === 'gustavax_0_0'); await page.keyboard.press('ArrowRight');
  assert.equal(await page.evaluate(() => document.activeElement.dataset.talent), 'gustavax_1_0', 'Right changes talent branch');
  await page.keyboard.press('Enter'); await page.waitForFunction(() => window.saranfou.inspect().state.players[0].progression.talents.includes('gustavax_1_0'));
  await page.locator('[data-talent="gustavax_1_0"].owned').waitFor();
  assert.equal(await page.locator('[data-talent="gustavax_1_0"] .talent-rank').textContent(), '1/1');
  assert.equal(await page.locator('.talent-links .lit').count(), 2);
  ...
  await page.waitForFunction(() => window.saranfou.inspect().state?.players[0].progression.talents.includes('gustavax_1_0'));
  assert.equal(await page.evaluate(() => localStorage.getItem('saranfou-talents-v1')), null);
  await page.keyboard.press('Escape'); await screen(page, 'pause');
