...
try {
  ...
  assert.match(await page.locator('#talent-progress').textContent(), /0\/6 TALENTS/);
  ...
  await page.keyboard.press('Escape'); await screen(page, 'pause'); await page.locator('#pause [data-action=evolution]').click();
  await screen(page, 'evolution'); await page.waitForFunction(() => document.activeElement?.dataset.talent === 'aplomb'); await page.keyboard.press('ArrowRight');
  assert.equal(await page.evaluate(() => document.activeElement.dataset.talent), 'chauffe', 'Right changes talent branch');
  await page.keyboard.press('Enter'); await page.waitForFunction(() => window.saranfou.inspect().state.players[0].progression.talents.includes('chauffe'));
  ...
  await page.keyboard.press('Escape'); await screen(page, 'pause'); await page.keyboard.press('Escape'); await screen(page, null);
  ...
  await guest.waitForFunction(() => window.saranfou.inspect().state.players[0].specialState?.duration === 9);
  await guest.waitForFunction(() => window.saranfou.inspect().state.players[0].maxHp === 202 && window.saranfou.inspect().state.players[0].speed === 299);
  ...
  await page.waitForFunction(() => window.saranfou.inspect().state?.players[0].progression.talents.includes('chauffe'));
  assert.equal(await page.evaluate(() => localStorage.getItem('saranfou-talents-v1')), null);
