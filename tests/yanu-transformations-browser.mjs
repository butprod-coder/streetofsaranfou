...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {YANU_BRANCHES}=await import('/game/yanu-talents.js');
    ...
    await Promise.all(["beast","fluo","plant","fx"].map(name=>assets.load(`/assets/heroes/talents/yanu/${name}.png`)));
    ...
    for(let branch=0;branch<3;branch++)for(const rank of [1,6]){
      const sim=new Simulation(['yanu']),p=sim.state.players[0];
      applyProfile(p,{milestones:TRANSFORMATION_MILESTONES,talents:YANU_BRANCHES[branch].nodes.slice(0,rank).map(n=>n.id)});
      ...
      const key=["yanuBeast","yanuFluo","yanuPlant"][branch];
    ...
    window.yanuQA={canvas,renderEvolution,profile:{milestones:TRANSFORMATION_MILESTONES,talents:[YANU_BRANCHES[1].nodes[0].id]}};
    return stats;
  });
  assert.ok(result.every(s=>s.transparent>.2));
  await page.screenshot({path:'test-results/yanu-transformations.png'});
  await page.evaluate(()=>{window.yanuQA.canvas.remove();document.querySelectorAll('.screen').forEach(e=>e.classList.remove('active'));document.querySelector('#evolution').classList.add('active');window.yanuQA.renderEvolution('yanu',window.yanuQA.profile,()=>{});});
  ...
  await page.screenshot({path:'test-results/yanu-talents-lock.png'});
