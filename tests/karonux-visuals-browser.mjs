...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {KARONUX_BRANCHES,KARONUX_MILESTONES}=await import('/game/karonux-talents.js');
    const {renderEvolution}=await import('/game/evolution-ui.js');
    const assets=new Assets();
    await Promise.all(['golf','ice','handi-combat','fx'].map(name=>assets.load(`/assets/heroes/talents/${name}.png`)));
    ...
    for(let branch=0;branch<3;branch++)for(const rank of [1,6]){
      const sim=new Simulation(['karonux']),p=sim.state.players[0];
      ...
      const key=['karonuxGolf','karonuxIce','karonuxHandi'][branch];
    ...
    window.karonuxQA={canvas,renderEvolution,profile:{milestones:KARONUX_MILESTONES,talents:[KARONUX_BRANCHES[1].nodes[0].id]}};
    return stats;
  });
  assert.ok(result.every(s=>s.transparent>.2));
  await page.screenshot({path:'test-results/karonux-transformations.png'});
  await page.evaluate(()=>{window.karonuxQA.canvas.remove();document.querySelectorAll('.screen').forEach(e=>e.classList.remove('active'));document.querySelector('#evolution').classList.add('active');window.karonuxQA.renderEvolution('karonux',window.karonuxQA.profile,()=>{});});
  ...
  await page.screenshot({path:'test-results/karonux-talents-lock.png'});
