...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {JUALOS_BRANCHES}=await import('/game/jualos-talents.js');
    ...
    await Promise.all(["commercial","pig","guitar","fx"].map(name=>assets.load(`/assets/heroes/talents/jualos/${name}.png`)));
    ...
    for(let branch=0;branch<3;branch++)for(const rank of [1,6]){
      const sim=new Simulation(['jualos']),p=sim.state.players[0];
      applyProfile(p,{milestones:TRANSFORMATION_MILESTONES,talents:JUALOS_BRANCHES[branch].nodes.slice(0,rank).map(n=>n.id)});
      ...
      const key=["jualosCommercial","jualosPig","jualosGuitar"][branch];
    ...
    window.jualosQA={canvas,renderEvolution,profile:{milestones:TRANSFORMATION_MILESTONES,talents:[JUALOS_BRANCHES[1].nodes[0].id]}};
    return stats;
  });
  assert.ok(result.every(s=>s.transparent>.2));
  await page.screenshot({path:'test-results/jualos-transformations.png'});
  await page.evaluate(()=>{window.jualosQA.canvas.remove();document.querySelectorAll('.screen').forEach(e=>e.classList.remove('active'));document.querySelector('#evolution').classList.add('active');window.jualosQA.renderEvolution('jualos',window.jualosQA.profile,()=>{});});
  ...
  await page.screenshot({path:'test-results/jualos-talents-lock.png'});
