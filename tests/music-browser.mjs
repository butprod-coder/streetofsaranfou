...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    for (const scene of ['menu', 'street', 'boss']) for (let chapter = 0; chapter < (scene === 'menu' ? 1 : 6); chapter++) {
      const duration = MUSIC.bars * 4 * 60 / (themeFor(scene, chapter).bpm + (scene === 'boss' ? MUSIC.bossTempo : 0));
      ...
      let at = .05, step = 0;
      while (step < MUSIC.bars * 16) { music.schedule(step, at, scene, chapter); at += stepDuration(scene, chapter, step++); }
      ...
      console.info(`Music rendered: ${scene} ${chapter + 1}, ${Math.round(duration)} s`);
      if (scene !== 'boss') previews.push(Array.from(pcm.slice(0, 24000 * 6), (x, i) => Math.round(Math.max(-1, Math.min(1, x)) * 32767 * Math.min(1, i / 480, (24000 * 6 - i) / 2400))));
    }
  ...
  console.log('PASS: 13 rendered arrangements / seven original themes, finite audible signal, no clipping, pause/mute/ducking, WAV preview.', result.reports);
