...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {LORENZO_BRANCHES}=await import('/game/lorenzo-talents.js');
    ...
    await Promise.all(['chimera','pigeon','skull','fx'].map(name=>assets.load(`/assets/heroes/talents/lorenzo/${name}.png`)));
    ...
    for(let branch=0;branch<3;branch++)for(const rank of [1,6]){
      const sim=new Simulation(['lorenzo']),p=sim.state.players[0];
      applyProfile(p,{milestones:TRANSFORMATION_MILESTONES,talents:LORENZO_BRANCHES[branch].nodes.slice(0,rank).map(n=>n.id)});
      ...
      const key=['lorenzoChimera','lorenzoPigeon','lorenzoSkull'][branch];
    ...
    window.lorenzoQA={canvas,renderEvolution,profile:{milestones:TRANSFORMATION_MILESTONES,talents:[LORENZO_BRANCHES[1].nodes[0].id]}};
    return stats;
  });
  assert.ok(result.every(s=>s.transparent>.2));
  await page.screenshot({path:'test-results/lorenzo-transformations.png'});
  await page.evaluate(()=>{window.lorenzoQA.canvas.remove();document.querySelectorAll('.screen').forEach(e=>e.classList.remove('active'));document.querySelector('#evolution').classList.add('active');window.lorenzoQA.renderEvolution('lorenzo',window.lorenzoQA.profile,()=>{});});
  ...
  await page.screenshot({path:'test-results/lorenzo-talents-lock.png'});
  await page.setViewportSize({width:390,height:844});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  assert.deepEqual(errors,[]);console.log('Browser QA passed: six forms, 48 nonempty sprite frames, four alpha atlases, 18 talents, branch lock, mobile.',result);
