...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {KIKOR_BRANCHES}=await import('/game/kikor-talents.js');
    ...
    await Promise.all(["painter","summoner","bike","minions","fx"].map(name=>assets.load(`/assets/heroes/talents/kikor/${name}.png`)));
    ...
    for(let branch=0;branch<3;branch++)for(const rank of [1,6]){
      const sim=new Simulation(['kikor']),p=sim.state.players[0];
      applyProfile(p,{milestones:TRANSFORMATION_MILESTONES,talents:KIKOR_BRANCHES[branch].nodes.slice(0,rank).map(n=>n.id)});
      ...
      const key=["kikorPainter","kikorSummoner","kikorBike"][branch];
    ...
    window.kikorQA={canvas,renderEvolution,profile:{milestones:TRANSFORMATION_MILESTONES,talents:[KIKOR_BRANCHES[1].nodes[0].id]}};
    return stats;
  });
  assert.ok(result.every(s=>s.transparent>.2));
  await page.screenshot({path:'test-results/kikor-transformations.png'});
  await page.evaluate(()=>{window.kikorQA.canvas.remove();document.querySelectorAll('.screen').forEach(e=>e.classList.remove('active'));document.querySelector('#evolution').classList.add('active');window.kikorQA.renderEvolution('kikor',window.kikorQA.profile,()=>{});});
  ...
  await page.screenshot({path:'test-results/kikor-talents-lock.png'});
