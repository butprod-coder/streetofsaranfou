...
test('two clients complete lobby, load barrier, inputs, shared pause, reload and retry', async t => {
  ...
  assert.equal(start.state.difficulty, 'hard'); assert.equal(start.state.players[0].progression.points, 1); assert.equal(start.state.players[1].progression.points, 1);
  const sim = game.rooms.get(hostInfo.code).sim;
  sim.awardTalentMilestone('street:0:0');
  ...
  host.send({ type: 'spend', slot: 1, stat: 'karonux_v3_0_0' });
  const upgraded = await host.next('state', m => m.state.players[0].progression.talents.includes('karonux_v3_0_0'));
  assert.equal(upgraded.state.players[1].progression.talents.length, 0, 'Slot spoof cannot spend another character’s points');
  guest.send({ type: 'spend', stat: 'karonux_v3_0_0' }); assert.match((await guest.next('error')).message, /points/);
  ...
  assert.equal(resume.state.players[0].progression.talents.includes('karonux_v3_0_0'), true, 'Reconnection retains investments');
