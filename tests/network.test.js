...
test('two clients complete lobby, load barrier, inputs, shared pause, reload and retry', async t => {
  ...
  host.send({ type: 'spend', slot: 1, stat: 'souffle' });
  const upgraded = await host.next('state', m => m.state.players[0].progression.talents.includes('souffle'));
  assert.equal(upgraded.state.players[1].progression.talents.length, 0, 'Slot spoof cannot spend another character’s points');
  guest.send({ type: 'spend', stat: 'souffle' }); assert.match((await guest.next('error')).message, /points/);
  ...
  assert.equal(resume.state.players[0].progression.talents.includes('souffle'), true, 'Reconnection retains investments');
