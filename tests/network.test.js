...
test('two clients complete lobby, load barrier, inputs, shared pause, reload and retry', async t => {
  ...
  host.send({ type: 'create', character: 'karonux', profile: { totalXp: 600, stats: { life: 1 } }, difficulty: 'hard' }); const hostInfo = await host.next('joined');
  ...
  assert.equal(start.state.difficulty, 'hard'); assert.equal(start.state.players[0].progression.level, 4); assert.equal(start.state.players[1].progression.level, 1);
  ...
  host.send({ type: 'spend', slot: 1, stat: 'attack' });
  const upgraded = await host.next('state', m => m.state.players[0].progression.stats.attack === 1);
  assert.equal(upgraded.state.players[1].progression.stats.attack, 0, 'Slot spoof cannot spend another character’s points');
  guest.send({ type: 'spend', stat: 'attack' }); assert.match((await guest.next('error')).message, /points/);
  ...
  assert.equal(resume.state.players[0].progression.stats.attack, 1, 'Reconnection retains investments');
  ...
  assert.equal(game.rooms.get(hostInfo.code).players[0].profile.stats.attack, 1, 'Continue retains investments');
});
