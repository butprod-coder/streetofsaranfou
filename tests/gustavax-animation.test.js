...
test('both punches and the kick use strike poses exactly when damage is created, then recover',()=>{
  ...
  g.updateGustavax(b,.5);assert.equal(b.pattern,null);assert.ok(b.recovering>0);assert.equal(gustavaxPose(b).frame,13);
});
...
test('a rolled chair starts at the boss lane and wall collision exposes him without repeated damage',()=>{
  ...
  assert.equal(g.state.hazards.length,0);assert.equal(b.pattern,null);assert.equal(b.recoveryKind,'angry');assert.ok(b.recovering>=3);assert.equal(gustavaxPose(b).frame,11);
});
...
test('two desk sweeps and final slam use their own frames, then Gustavax falls to his knees',()=>{
  ...
  g.updateGustavax(b,.6);assert.equal(b.pattern,null);assert.equal(gustavaxPose(b).frame,14);assert.ok(b.recovering>=3.3);
});
