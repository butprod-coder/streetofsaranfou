...
import { TALENTS, normalizeProfile, spendPoint, spendAttribute, applyProfile, xpForLevel, TALENT_MILESTONES } from '../game/progression.js';
...
test('all 21 ultimate paths run finite in solo and serialize without errors',()=>{
...
test('per-character records deduplicate repeated snapshots and do not grant starting power',()=>{
  ...
  const r=JSON.parse(storage.getItem(RECORDS_KEY)).jo;assert.equal(r.xp,250);assert.equal(r.wins,1);assert.ok(r.unlocks.includes('first-victory'));assert.equal(new Simulation(['jo']).state.players[0].progression.level,1);
});
