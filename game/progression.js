...
export function canLearn(profile, node) {
  ...
  if (node.ultimate && (profile.completed.length < 3 || branch.some(n => n.ultimate && profile.talents.includes(n.id)))) return false;
  return true;
}
export function normalizeProfile(raw = {}, kind = raw?.kind || 'karonux') {
  ...
  const total = Math.floor((level - 1) / 2) + completed.filter(n => n < 5).length;
  const p = { kind, xp, level, completed, talents: [], points: total, attributes: {}, statPoints: (level - 1) * 2 };
...
export function bonuses(profile) {
  ...
  if (hasTalent(actor, 'Couenne épaisse')) b.stagger *= .65;
