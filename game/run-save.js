...
import { ENCOUNTER_ROSTER } from './encounters.js';
...
export function checkpoint(state) {
  ...
  return validateCheckpoint({ version: 2, runId: state.runId, chapter: state.chapter, stage: state.stage, wave: state.wave, phase: state.phase,
    ...
    difficulty: state.difficulty, score: state.score, kills: state.kills, bestCombo: state.bestCombo,
    players: state.players.map(p => ({ kind: p.kind, profile: p.progression, health: p.hp / p.maxHp, energy: p.energy, lives: p.lives,
      weapon: p.weapon, rewards: {}, choices: [], supportRole: p.supportRole, sleepSaveChapter: p.sleepSaveChapter })),
    props: state.props.filter(p => p.kind !== 'easel').map(p => [p.id, p.hp]),
    pickups: state.pickups.map(p => ({ kind: p.kind, weapon: p.weapon, uses: p.uses, x: p.x, y: p.y })),
  });
}
...
export function validateCheckpoint(raw) {
  if (!raw || raw.version !== 2 || !Array.isArray(raw.players) || raw.players.length < 1 || raw.players.length > 2 || !['intro','rest','clear'].includes(raw.phase) || !Number.isInteger(raw.chapter) || raw.chapter < 0 || raw.chapter > 5 || !Number.isInteger(raw.stage) || raw.stage < 0 || raw.stage > 5) throw new Error('Sauvegarde incompatible ou invalide.');
  ...
  const players = raw.players.map(p => {
    ...
    return { kind: p.kind, profile: normalizeProfile(p.profile, p.kind), health: number(p.health, 1), energy: number(p.energy,100), lives: integer(p.lives,5),
      ...
      rewards, choices, supportRole: ['heal','guard','attack'].includes(p.supportRole) ? p.supportRole : 'attack', sleepSaveChapter: integer(p.sleepSaveChapter,5,-1) };
  });
  if (!players.some(p => p.health > 0)) throw new Error('Cette sortie est terminée.');
  return { version:2, runId:raw.runId, chapter:raw.chapter, stage:raw.stage, phase:raw.phase, wave:integer(raw.wave,4,-1), time:number(raw.time,86400),
    seed:integer(raw.seed,0xffffffff), streetSeed:integer(raw.streetSeed,0xffffffff), streetBag:bag(raw.streetBag), nextId:integer(raw.nextId,1000000,10),
    ...
    pickups:(Array.isArray(raw.pickups)?raw.pickups:[]).slice(0,30).filter(p=>p&&['food','energy','weapon'].includes(p.kind)&& (p.kind!=='weapon'||Object.hasOwn(WEAPONS,p.weapon))).map(p=>({kind:p.kind,weapon:p.kind==='weapon'?p.weapon:undefined,uses:integer(p.uses,24),x:number(p.x,1230,50),y:number(p.y,660,450)})),
  };
}
export function restoreCheckpoint(raw) {
  ...
  save.players.forEach((p,i)=>{
    const actor=s.players[i];actor.rogueRewards={};refreshPlayerStats(actor);actor.hp=actor.maxHp*p.health;actor.energy=p.energy;actor.lives=p.lives;actor.weapon=p.weapon;
