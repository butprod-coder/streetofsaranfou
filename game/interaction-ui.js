export function installInteractionUI() {
  ...
  for (const [key, text] of [['Contact', 'Saisie automatique · poing + direction arrière pour projeter'], ['F / RT ou R2', 'Ramasser / échanger une arme · à vide, la déposer'], ['J / X manette avec arme', 'Frapper / tirer · usages et munitions limités']]) {
  ...
  for (const [key, label, text] of [['interact', 'Ramasser une arme', 'F']]) {
  ...
  document.querySelector('.keyboard-hint').innerHTML = 'J <i>FRAPPER / ARME</i> K <i>PIED</i> L <i>SPÉCIAL</i> J + ←/→ <i>PROJECTION</i> F <i>RAMASSER</i>';
