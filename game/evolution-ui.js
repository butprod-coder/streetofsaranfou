...
export function installEvolutionUI() {
  ...
  document.querySelector('.tip').textContent = 'Lis les gestes des ennemis : saute ou esquive avant le coup. Seuls les bidons explosifs annoncent leur zone au sol. XP → caractéristiques. Fin de la première rue → premier talent. Coups réussis → énergie ; barre pleine → spécial. Pause → Progression. Manettes : X/□ poing, Y/△ pied, A/✕ saut, B/○ spécial, RB/R1 esquive.';
}
...
export function renderPauseTalents(player) {
  ...
  const attributes = document.querySelector('#pause-attributes');
  attributes.classList.toggle('has-points', profile.statPoints > 0);
