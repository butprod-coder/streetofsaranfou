...
import { CHAPTERS, FIGHTERS, ENEMIES, fighter, clamp } from './data.js';
import { BALANCE } from './balance.js';
import { applyProfile, xpForLevel, TALENT_MILESTONES } from './progression.js';
import { KARONUX_MILESTONES } from './karonux-talents.js';

export const SECRET_ENEMIES = Object.entries(ENEMIES).sort((a,b)=>a[1].name.localeCompare(b[1].name,'fr'));
...
export function createSecretSession(options={}){
  const chapter=options.mode==='enemy'?0:clamp(Math.trunc(Number(options.chapter))||0,0,6),character=fighter(options.character).id;
  const mode=['boss','enemy'].includes(options.mode)?options.mode:'level',stage=clamp(Math.trunc(Number(options.stage))||0,0,chapter===6?6:5);
  const settings={chapter,character,mode,stage,enemy:Object.hasOwn(ENEMIES,options.enemy)?options.enemy:SECRET_ENEMIES[0][0],phase:Number(options.phase)||0,difficulty:['easy','normal','hard'].includes(options.difficulty)?options.difficulty:'normal',invulnerable:options.invulnerable===true,freeSpecial:options.freeSpecial===true,cinema:options.cinema!==false,boost:options.boost===true};
  ...
  if(mode==='enemy'){
    const s=sim.state;
    Object.assign(s,{stage:0,phase:'fight',phaseTime:0,wave:0,waves:[{kinds:[settings.enemy],label:'TEST ENNEMI',boss:false}],enemies:[],spawnQueue:[],hazards:[],allies:[],props:[],pickups:[],surprise:null,surpriseDone:true,bossCinema:null});
    Object.assign(s.players[0],{x:460,y:550,lives:0});
    sim.spawnEnemy(settings.enemy,{x:800,y:550,cooldown:1});
  }
  if(settings.boost)applyProfile(sim.state.players[0],{kind:character,xp:xpForLevel(20),milestones:usesTransformationTree(character)?TRANSFORMATION_MILESTONES:TALENT_MILESTONES},false);
  return sim;
}
...
export function installSecretMenu(game){
  ...
  const sync=()=>{
    ...
    const enemy=$('secret-mode').value==='enemy';
    $('secret-enemy-label').hidden=!enemy;$('secret-chapter-label').hidden=enemy;$('secret-cinema').closest('label').hidden=enemy;
    $('secret-start-label').hidden=boss||enemy;$('secret-phase-label').hidden=!boss;
